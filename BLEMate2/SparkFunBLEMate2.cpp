...
BLEMate2::opResult BLEMate2::BLEStatus(int &status){
    String buffer;
    String EOL = String("\n\r");
    ...
    while ((startTime + 3000) > millis())
    {
      ...
      if (buffer.endsWith(EOL))
      {
        ...
        {
          if (buffer.find("INIT") > 0){
            status = 1;
          }
          else if(buffer.find("ADV") > 0){
              status =  2;
          }
          else if(buffer.find("SCN") > 0){
             status =  3;
          }
          else if(buffer.find("CON") > 0){
             status =  4;
          }
          else if(buffer.find("IDL") > 0){
            status =  5;
          }
        }
        buffer = "";
